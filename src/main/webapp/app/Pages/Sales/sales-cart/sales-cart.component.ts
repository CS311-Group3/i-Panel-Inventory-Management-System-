...
export class SalesCartComponent implements OnInit {
  ...
  constructor(private inventoryService: InventoryService, private modalService: NgbModal, public salesData: SalesData) {
    this.itemName = '';
    this.itemCode = '';
  }
  ...
  search(): void {
    if ((this.itemName === '' || this.itemName.length === 0) && (this.itemCode === '' || this.itemCode.length === 0)) {
      ...
      this.inventoryService
        .findAllByCode(this.itemCode)
        .subscribe((res: HttpResponse<IInventory[]>) => (this.searchItems = res.body || []));
    } else if (this.itemCode === '' || this.itemCode.length === 0) {
      this.inventoryService
        .findAllByName(this.itemName)
        .subscribe((res: HttpResponse<IInventory[]>) => (this.searchItems = res.body || []));
    } else {
      this.inventoryService
        .findAllByCodeAndName(this.itemCode, this.itemName)
        .subscribe((res: HttpResponse<IInventory[]>) => (this.searchItems = res.body || []));
    }
  }
